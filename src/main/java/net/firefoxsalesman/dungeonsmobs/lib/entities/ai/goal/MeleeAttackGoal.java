package net.firefoxsalesman.dungeonsmobs.lib.entities.ai.goal;
		...
		if (i - this.lastCanUseCheck < 20L) {
			...
			if (livingentity == null) {
				...
				if (this.path != null) {
					...
					return this.getAttackReachSqr(livingentity) >= this.mob.distanceToSqr(
		...
		if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(livingentity))
			...
			if (this.canPenalize) {
				...
				if (this.mob.getNavigation().getPath() != null) {
					net.minecraft.world.level.pathfinder.Node finalPathPoint = this.mob
							.getNavigation().getPath().getEndNode();
