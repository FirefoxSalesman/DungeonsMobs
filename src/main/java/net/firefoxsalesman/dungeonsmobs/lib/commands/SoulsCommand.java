...
public class SoulsCommand {
	private static final SimpleCommandExceptionType ERROR_ADD_FAILED = new SimpleCommandExceptionType(
			Component.translatable("commands.souls.add.failed"));
	private static final SimpleCommandExceptionType ERROR_SET_FAILED = new SimpleCommandExceptionType(
			Component.translatable("commands.souls.set.failed"));

	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		LiteralArgumentBuilder<CommandSourceStack> mobEnchantCommand = Commands.literal("souls")
				.requires((commandSource) -> commandSource.hasPermission(2))
				.then(Commands.literal("add")
						.then(Commands.argument("targets", EntityArgument.players())
								.then(Commands.argument("amount",
										IntegerArgumentType.integer())
										.executes((p_198445_0_) -> addSouls(
												p_198445_0_.getSource(),
												EntityArgument.getPlayers(
														p_198445_0_,
														"targets"),
												IntegerArgumentType
														.getInteger(p_198445_0_,
																"amount"))))))
				.then(Commands.literal("set")
						.then(Commands.argument("targets", EntityArgument.players())
								.then(Commands.argument("amount",
										IntegerArgumentType.integer())
										.executes((p_198445_0_) -> setSouls(
												p_198445_0_.getSource(),
												EntityArgument.getPlayers(
														p_198445_0_,
														"targets"),
	...
	private static int addSouls(CommandSourceStack source, Collection<ServerPlayer> targets, int amount)
	...
	private static int setSouls(CommandSourceStack source, Collection<ServerPlayer> targets, int amount)
			throws CommandSyntaxException {
		targets.forEach(serverPlayer -> {
