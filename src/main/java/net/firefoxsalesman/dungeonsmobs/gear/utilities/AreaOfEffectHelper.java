...
public class AreaOfEffectHelper {
	...
	public static void pullInNearbyEntities(LivingEntity attacker, LivingEntity target, float distance,
		...
		applyToNearbyEntities(target, distance,
				...
				(LivingEntity nearbyEntity) -> pullVictimTowardsTarget(target, nearbyEntity,
						particleType, PULL_IN_SPEED_FACTOR));
	}

	public static void pullVictimTowardsTarget(LivingEntity target, LivingEntity nearbyEntity,
			SimpleParticleType particleType, double pullInSpeedFactor) {
	...
	public static void pullInNearbyEntitiesAtPos(LivingEntity attacker, BlockPos blockPos, int distance,
		...
		applyToNearbyEntitiesAtPos(blockPos, world, distance,
				...
				(LivingEntity nearbyEntity) -> pullVictimTowardsPos(blockPos, nearbyEntity,
						particleType));
	}

	private static void pullVictimTowardsPos(BlockPos targetPos, LivingEntity nearbyEntity,
			SimpleParticleType particleType) {
	...
	public static void chainNearbyEntities(LivingEntity attacker, LivingEntity target, float distance,
			int timeMultiplier) {
		PROXY.spawnParticles(target, ParticleTypes.PORTAL);
				...
				(LivingEntity nearbyEntity) -> {
					if (nearbyEntity.getHealth() < nearbyEntity.getMaxHealth()) {
						if (potionEffect.getEffect().isInstantenous()) {
							potionEffect.getEffect().applyInstantenousEffect(playerentity,
									playerentity, nearbyEntity,
	...
	public static LivingEntity findMostInjuredAlly(LivingEntity healer, float distance) {
		...
		List<LivingEntity> nearbyEntities = getNearbyEnemies(healer, distance, world,
				getCanHealPredicate(healer));
		if (!nearbyEntities.isEmpty()) {
			...
			for (LivingEntity injure : nearbyEntities) {
				if (mostInjuredAlly == null
						|| injure.getMaxHealth() - injure.getHealth() > lostHealth) {
	...
	public static void causeShockwave(LivingEntity attacker, LivingEntity target, float damageAmount,
			float distance) {
		...
		applyToNearbyEntities(target, distance,
				(nearbyEntity) -> isFacingEntity(attacker, nearbyEntity, vec1.subtract(vec2), 60)
						&& getCanApplyToSecondEnemyPredicate(attacker, target)
								.test(nearbyEntity),
				(LivingEntity nearbyEntity) -> nearbyEntity.hurt(shockwave, damageAmount));
	}

	public static void causeExplosionAttack(LivingEntity attacker, LivingEntity target, float damageAmount,
			float distance) {
		DamageSource explosion = target.damageSources().explosion(attacker, target);
		applyToNearbyEntities(target, distance,
				getCanApplyToEnemyPredicate(attacker),
				(LivingEntity nearbyEntity) -> nearbyEntity.hurt(explosion, damageAmount));
	}

	public static void causeMagicExplosionAttack(LivingEntity attacker, LivingEntity target, float damageAmount,
			float distance) {
		DamageSource magicExplosion = attacker.damageSources().indirectMagic(attacker, target);
		applyToNearbyEntities(target, distance,
	...
	public static void burnNearbyEnemies(LivingEntity attacker, float damage, float distance) {
		applyToNearbyEntities(attacker, distance,
				getCanApplyToEnemyPredicate(attacker), (LivingEntity nearbyEntity) -> {
					nearbyEntity.hurt(attacker.damageSources().onFire(), damage);
	...
	public static void causeSwirlingAttack(Player attacker, LivingEntity target, float damageAmount,
			float distance) {
		DamageSource swirling = attacker.damageSources().playerAttack(attacker);
		applyToNearbyEntities(attacker, distance,
	...
	public static void causeEchoAttack(LivingEntity attacker, LivingEntity target, float damageAmount,
			float distance, final int echoLevel) {
		applyToNearbyEntities(target, distance, echoLevel,
				getCanApplyToSecondEnemyPredicate(attacker, target),
				(LivingEntity nearbyEntity) -> {
					DamageSource echo = attacker.damageSources().mobAttack(attacker);
					if (attacker instanceof Player) {
						echo = attacker.damageSources().playerAttack((Player) attacker);
					}
	...
	public static void poisonAndSlowNearbyEnemies(Level worldIn, Player playerIn, int distance) {
		applyToNearbyEntities(playerIn, worldIn, distance,
				getCanApplyToEnemyPredicate(playerIn),
				(LivingEntity nearbyEntity) -> {
					MobEffectInstance entangled = new MobEffectInstance(
							MobEffects.MOVEMENT_SLOWDOWN, 140, 4);
	...
	public static void weakenAndMakeNearbyEnemiesVulnerable(Player playerIn, Level world, int distance) {
		applyToNearbyEntities(playerIn, world, distance,
				...
				(LivingEntity nearbyEntity) -> {
					...
					MobEffectInstance vulnerability = new MobEffectInstance(
							MobEffects.DAMAGE_RESISTANCE, 140, -2);
