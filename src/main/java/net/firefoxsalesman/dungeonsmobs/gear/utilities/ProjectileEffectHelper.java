...
public class ProjectileEffectHelper {
	...
	public static void ricochetArrowTowardsOtherEntity(LivingEntity attacker, LivingEntity victim,
			AbstractArrow arrowEntity, int distance) {
		...
		List<LivingEntity> nearbyEntities = world.getEntitiesOfClass(LivingEntity.class,
				victim.getBoundingBox().inflate(distance),
				getCanApplyToSecondEnemyPredicate(attacker, victim));
	...
	public static void fireBonusShotTowardsOtherEntity(LivingEntity attacker, int distance,
			double bonusShotDamageMultiplier, float arrowVelocity) {
		Level world = attacker.getCommandSenderWorld();
		// boolean nullListFlag = arrowEntity.hitEntities == null;
		List<LivingEntity> nearbyEntities = world.getEntitiesOfClass(LivingEntity.class,
				new AABB(attacker.getX() - distance, attacker.getY() - distance,
						attacker.getZ() - distance,
						attacker.getX() + distance, attacker.getY() + distance,
						attacker.getZ() + distance),
		...
		if (target != null) {
			...
			double euclideanDist = Mth.sqrt((float) (towardsX * towardsX + towardsZ * towardsZ));
			double towardsY = target.getY(0.3333333333333333D) - arrowEntity.getY()
					+ euclideanDist * (double) 0.2F;
			arrowEntity.shootFromRotation(attacker, attacker.getXRot(), attacker.getYRot(), 0.0F,
					arrowVelocity * 3.0F, 1.0F);
			setProjectileTowards(arrowEntity, towardsX, towardsY, towardsZ, 0);
			arrowEntity.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
			// arrowEntity.addTag("BonusProjectile"); // Commented this out because it
			// should no longer be used, if a user reports a bug about this, uncomment this
			attacker.level().addFreshEntity(arrowEntity);
		}
	}

	public static void fireBurstBowstringShots(LivingEntity attacker, int distance, double damageMultiplier,
			float arrowVelocity, int arrowsToFire) {
		Level world = attacker.getCommandSenderWorld();
		// boolean nullListFlag = arrowEntity.hitEntities == null;
		List<LivingEntity> nearbyEntities = world.getEntitiesOfClass(LivingEntity.class,
				new AABB(attacker.getX() - distance, attacker.getY() - distance,
	...
	public static void fireSnowballAtNearbyEnemy(LivingEntity attacker, int distance) {
		...
		if (target != null) {
			...
			double towardsZ = target.getZ() - attacker.getZ();
			double euclideanDist = Mth.sqrt((float) (towardsX * towardsX + towardsZ * towardsZ));
			double towardsY = target.getY(0.3333333333333333D) - snowballEntity.getY()
					+ euclideanDist * (double) 0.2F;
			snowballEntity.shootFromRotation(attacker, attacker.getXRot(), attacker.getYRot(), 0.0F, 1.5F,
					1.0F);
	...
	private static AbstractArrow createChainReactionProjectile(Level world, LivingEntity attacker,
			ItemStack ammoStack, AbstractArrow originalArrow) {
		ArrowItem arrowItem = (ArrowItem) (ammoStack.getItem() instanceof ArrowItem ? ammoStack.getItem()
				: Items.ARROW);
	...
	public static void fireChainReactionProjectiles(Level world, LivingEntity attacker, LivingEntity victim,
			float v, float v1, AbstractArrow originalArrow) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (!currentProjectile.isEmpty()) {
				if (i == 0) {
					fireChainReactionProjectileFromVictim(world, attacker, victim,
							currentProjectile, randomSoundPitches[1], v, v1, 45.0F,
							originalArrow);
				} else if (i == 1) {
					fireChainReactionProjectileFromVictim(world, attacker, victim,
							currentProjectile, randomSoundPitches[2], v, v1, -45.0F,
							originalArrow);
				} else if (i == 2) {
					fireChainReactionProjectileFromVictim(world, attacker, victim,
							currentProjectile, randomSoundPitches[1], v, v1, 135.0F,
							originalArrow);
				} else if (i == 3) {
					fireChainReactionProjectileFromVictim(world, attacker, victim,
							currentProjectile, randomSoundPitches[2], v, v1, -135.0F,
							originalArrow);
				}
			}
		}
	...
	private static void fireChainReactionProjectileFromVictim(Level world, LivingEntity attacker,
			LivingEntity victim, ItemStack projectileStack, float soundPitch, float v1, float v2,
			float centerOffset, AbstractArrow originalArrow) {
		if (!world.isClientSide) {
			...
			Quaternionf quaternion = new Quaternionf(upVector.x(), upVector.y(), upVector.z(), centerOffset)
					.mul((float) upVector.x(), (float) upVector.y(), (float) upVector.z(),
							centerOffset);
			Vec3 lookVector = victim.getViewVector(1.0F);
			Vector3f vector3f = new Vector3f((float) lookVector.x(), (float) lookVector.y(),
					(float) lookVector.z());
			quaternion.transform(vector3f);
			projectile.shoot(vector3f.x(), vector3f.y(), vector3f.z(), v1, v2);
			world.addFreshEntity(projectile);
			world.playSound(null, victim.getX(), victim.getY(), victim.getZ(), SoundEvents.CROSSBOW_SHOOT,
					attacker.getSoundSource(), 1.0F, soundPitch);
		}
	}
	...
	// Chief: it's a copy-paste of ProjectileEntity::shoot that creates a new
	// Random. Why?
	// Infamous: This one doesn't require a velocity parameter, but I imagine I
	// still added this unnecessarily
	// Infamous: Removed the Random instantiation and made it a constant
	public static void setProjectileTowards(Projectile projectileEntity, double x, double y, double z,
			float inaccuracy) {
		...
		projectileEntity.setYRot(
				(float) (Mth.atan2(vector3d.x, vector3d.z) * (double) (180F / (float) Math.PI)));
	...
	public static List<LivingEntity> rayTraceEntities(Level worldIn, Vec3 startVec, Vec3 endVec, AABB boundingBox,
