...
public class ArmorEffectHelper {
	public static void summonOrTeleportBat(Player playerEntity, Level world) {
		if (world instanceof ServerLevel) {
			Leader summonerCap = getLeaderCapability(playerEntity);
			List<Entity> batSummons = summonerCap.getSummonedMobs().stream()
					.filter(entity -> entity.getType() == EntityType.BAT).toList();
			if (batSummons.isEmpty()) {
				...
				batPet.teleportToWithTicket(playerEntity.getX() + playerEntity.getEyeHeight(),
						playerEntity.getY() + playerEntity.getEyeHeight(),
						playerEntity.getZ() + playerEntity.getEyeHeight());
			}
	...
	public static void teleportOnHit(LivingEntity livingEntity) {
		...
		if (!world.isClientSide) {
			...
			for (int i = 0; i < 16; ++i) {
				double teleportX = livingEntity.getX()
				...
				double teleportZ = livingEntity.getZ()
						+ (livingEntity.getRandom().nextDouble() - 0.5D) * 16.0D;
				...
				if (livingEntity.randomTeleport(teleportX, teleportY, teleportZ, true)) {
					...
					world.playSound(null, livingEntity.blockPosition(), soundEvent,
	...
	// public static void handleJumpEnchantments(Player playerEntity, ItemStack
	// helmet, ItemStack chestplate) {
