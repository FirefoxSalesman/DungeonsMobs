package net.firefoxsalesman.dungeonsmobs.gear.client.renderer;
...
public class BeamEntityRenderer<T extends ArtifactBeamEntity> extends EntityRenderer<T> {
	...
	private static void drawBeams(double distance, ArtifactBeamEntity entity, float ticks, float speedModifier,
		...
		pMatrixStack.mulPose(Axis.YP.rotationDegrees(
				(Mth.lerp(ticks, boundDegrees(-entity.getYRot()), boundDegrees(-entity.yRotO)))));
		pMatrixStack.mulPose(Axis.XP.rotationDegrees(
				Mth.lerp(ticks, boundDegrees(entity.getXRot()), boundDegrees(entity.xRotO))));
		...
		drawClosingBeam(builder, positionMatrix, matrixNormal, additiveThickness, distance / 10, 0.5, 1, ticks,
				beam2r, beam2g, beam2b, 0.9f);

		// main laser, colored part
		builder = buffer.getBuffer(MyRenderType.BEACON_BEAM_MAIN);
		drawBeam(builder, positionMatrix, matrixNormal, entity.getBeamWidth(), distance, v, v + distance * 1.5,
				ticks, beam2r, beam2g, beam2b, 0.7f);

		// core
		builder = buffer.getBuffer(MyRenderType.BEACON_BEAM_CORE);
		drawBeam(builder, positionMatrix, matrixNormal, entity.getBeamWidth() * 0.7f, distance, v,
				v + distance * 1.5, ticks, beam1r, beam1g, beam1b, 1f);
		pMatrixStack.popPose();
		buffer.endBatch();
	}

	private static float boundDegrees(float v) {
		// Time wasted trying to get beams not jumping around due to networking: TOO
		// DAMN LONG!
	...
	private static void drawBeam(VertexConsumer builder, Matrix4f positionMatrix, Matrix3f matrixNormalIn,
			float thickness, double distance, double v1, double v2, float ticks, float r, float g, float b,
			float alpha) {
		Vector3f vector3f = new Vector3f(0.0f, 1.0f, 0.0f);
		vector3f.mulTranspose(matrixNormalIn);
	...
	private static void drawClosingBeam(VertexConsumer builder, Matrix4f positionMatrix, Matrix3f matrixNormalIn,
			float thickness, double distance, double v1, double v2, float ticks, float r, float g, float b,
			float alpha) {
		Vector3f vector3f = new Vector3f(0.0f, 1.0f, 0.0f);
		vector3f.mulTranspose(matrixNormalIn);
	...
	private static void drawQuad(VertexConsumer builder, float v1, float v2, float r, float g, float b, float alpha,
			Vector3f vector3f, Vector4f vec1, Vector4f vec2, Vector4f vec3, Vector4f vec4) {
		builder.vertex(vec4.x(), vec4.y(), vec4.z(), r, g, b, alpha, 0, v1, OverlayTexture.NO_OVERLAY, 15728880,
				vector3f.x(), vector3f.y(), vector3f.z());
		builder.vertex(vec3.x(), vec3.y(), vec3.z(), r, g, b, alpha, 0, v2, OverlayTexture.NO_OVERLAY, 15728880,
				vector3f.x(), vector3f.y(), vector3f.z());
		builder.vertex(vec2.x(), vec2.y(), vec2.z(), r, g, b, alpha, 1, v2, OverlayTexture.NO_OVERLAY, 15728880,
				vector3f.x(), vector3f.y(), vector3f.z());
		builder.vertex(vec1.x(), vec1.y(), vec1.z(), r, g, b, alpha, 1, v1, OverlayTexture.NO_OVERLAY, 15728880,
				vector3f.x(), vector3f.y(), vector3f.z());
	}
}
