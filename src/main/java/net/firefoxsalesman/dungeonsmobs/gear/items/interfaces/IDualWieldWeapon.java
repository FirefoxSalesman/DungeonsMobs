...
public interface IDualWieldWeapon<T extends Item> {
	default boolean canAttack(LivingEntity e, ItemStack is) {
		return e.getMainHandItem().getItem() instanceof IDualWieldWeapon
				&& e.getOffhandItem().getItem() instanceof IDualWieldWeapon;
	}

	/*
	 * logic: if on main hand and offhand is not dummy, wrap offhand into dummy
	 * dummy tracks and copies the stack tags of the itemstack it was linked with
	 * via dual wield capability attached
	 * damage taken by dummy is forwarded to main
	 * if dummy detects main hand is not correct itemstack, or is not in the offhand
	 * slot, it unwraps itself
	 */
	default ItemStack wrapStack(ItemStack wrapper, ItemStack content) {
		wrapper.getCapability(DUAL_WIELD_CAPABILITY)
				.ifPresent((a) -> a.setWrappedItemStack(content));
		return wrapper;
	}

	default ItemStack unwrapStack(ItemStack wrapped) {
		if (!wrapped.getCapability(DUAL_WIELD_CAPABILITY).isPresent())
			return ItemStack.EMPTY;
		return wrapped.getCapability(DUAL_WIELD_CAPABILITY).resolve().get()
				.getWrappedItemStack();
	}
	...
	default void copyTag(ItemStack from, ItemStack to) {
		...
		// to.getCapability(ModCapabilities.DUAL_WIELD_CAPABILITY).ifPresent((a) ->
		// a.setFake(true));
	}

	default ItemStack createDummy(ItemStack of) {
		ItemStack ret = of.copy();
		ret.getCapability(DUAL_WIELD_CAPABILITY).ifPresent((a) -> {
	...
	default void updateMain(LivingEntity wielder, ItemStack is) {
		...
		if (wielder.getOffhandItem().getItem() != is.getItem()) {
			wielder.setItemInHand(InteractionHand.OFF_HAND,
					wrapStack(createDummy(is), wielder.getOffhandItem()));
	...
	default void updateOff(LivingEntity wielder, ItemStack is) {
		...
		is.getCapability(DUAL_WIELD_CAPABILITY).ifPresent((a) -> {
			if (!a.isFake())
				return;
			if (a.getLinkedItemStack().isEmpty() || wielder.getMainHandItem() != a.getLinkedItemStack())
				...
				copyTag(a.getLinkedItemStack(), is);
		});
	}
	...
	default void updateInventory(Player e, ItemStack stack, int slot) {
		...
		stack.getCapability(DUAL_WIELD_CAPABILITY).ifPresent((a) -> {
			if (a.isFake() && e.getCapability(ForgeCapabilities.ITEM_HANDLER, Direction.UP).isPresent()) {
				final IItemHandler mainInventory = e
						.getCapability(ForgeCapabilities.ITEM_HANDLER, Direction.UP).resolve()
						.get();
				// If this item is in their main inventory and it can be extracted.
				if (mainInventory.getStackInSlot(slot) == stack
						&& !mainInventory.extractItem(slot, stack.getCount(), true).isEmpty()) {
