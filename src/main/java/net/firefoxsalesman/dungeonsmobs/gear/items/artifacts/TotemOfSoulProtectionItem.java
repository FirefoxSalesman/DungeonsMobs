	...
	public InteractionResultHolder<ItemStack> procArtifact(ArtifactUseContext itemUseContext) {
		...
		if (world.isClientSide || itemUseContext.isHitMiss()) {
			...
			if (itemUseContextPlayer != null) {
				if (SoulCasterHelper.consumeSouls(itemUseContextPlayer,
						this.getActivationCost(itemUseContextItem))) {
					TotemOfSoulProtectionEntity totemOfSoulProtectionEntity = EntityTypeInit.TOTEM_OF_SOUL_PROTECTION
					...
					if (totemOfSoulProtectionEntity != null) {
						...
						itemUseContextPlayer.level()
								.addFreshEntity(totemOfSoulProtectionEntity);
