...
public class ArtifactBeamEntity extends Entity implements IEntityAdditionalSpawnData {
	...
	public ArtifactBeamEntity(EntityType<?> p_i48580_1_, BeamColor beamColor, Level p_i48580_2_,
			LivingEntity owner) {
		super(p_i48580_1_, p_i48580_2_);
		this.setOwner(owner);
	...
	public void tick() {
		...
		if (!this.level().isClientSide) {
			...
			if (!artifactUsage.isUsingArtifact() || !(artifactUsage.getUsingArtifact()
					.getItem() instanceof AbstractBeaconItem)) {
				this.remove(RemovalReason.DISCARDED);
				return;
			}
		}
		...
		if (!this.level().isClientSide()) {
			...
			while (true) {
				if (this.position().distanceTo(aabb.getCenter()) > distanceToDestination
						|| this.position().distanceTo(aabb.getCenter()) > MAX_RAYTRACE_DISTANCE)
					break;
				entities.addAll(this.level().getEntitiesOfClass(LivingEntity.class, aabb,
						getCanApplyToEnemyPredicate(owner)));
				distanceTraveled += 1.0d;
				Vec3 viewVector = this.getViewVector(1.0F);
				Vec3 targetVector = this.position().add(viewVector.x * distanceTraveled,
						viewVector.y * distanceTraveled, viewVector.z * distanceTraveled);
				aabb = new AABB(targetVector, targetVector).inflate(1.0D);
			}
			for (LivingEntity entity : entities) {
				entity.invulnerableTime = 0;
				Vec3 deltaMovement = entity.getDeltaMovement();
				entity.hurt(entity.damageSources().magic(), BEAM_DAMAGE_PER_TICK);
	...
	public HitResult beamTraceResult(double distance, float ticks, boolean passesWater) {
		...
		return level().clip(new ClipContext(vector3d, vector3d2, ClipContext.Block.COLLIDER,
				passesWater ? ClipContext.Fluid.ANY : ClipContext.Fluid.NONE, this));
	}
