...
public class SoulWizardEntity extends AbstractGolem implements GeoEntity {
	...
	public SoulWizardEntity(EntityType<? extends SoulWizardEntity> type, Level level) {
		super(type, level);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(1,
				new NearestAttackableTargetGoal<>(this, Mob.class, 1, false, false, (p_234199_0_) -> {
	...
	public void baseTick() {
		...
		if (this.getDeltaMovement().x < -0.05) {
			...
			moving = this.getDeltaMovement().z >= 0.05;

		if (moving) {
			this.level().addParticle(ParticleInit.SOUL_DUST.get(), this.getX(), this.getY(), this.getZ(), 0,
					0, 0);
		...
		if (!this.level().isClientSide) {
			...
			if (this.hasDelayedForm()) {
				...
				((ServerLevel) level()).sendParticles(ParticleInit.SOUL_DUST.get(), this.getX(),
						this.getY(), this.getZ(), 20, 0.5D, 1.0D, 0.5D, 0.0D);
			...
			if (this.getAliveFor() >= 800) {
				...
				((ServerLevel) this.level()).sendParticles(ParticleInit.SOUL_DUST.get(), this.getX(),
						this.getY(), this.getZ(), 20, 0.5D, 1.0D, 0.5D, 0.0D);
			}
		}
	}
	...
	private <P extends GeoAnimatable> PlayState predicate(AnimationState<P> event) {
		if (this.appearAnimationTick > 0) {
			event.getController()
					.setAnimation(RawAnimation.begin().then("soul_wizard_appear", PLAY_ONCE));
		} else if (this.shootAnimationTick > 0) {
			event.getController()
					.setAnimation(RawAnimation.begin().then("soul_wizard_attack", PLAY_ONCE));
		} else if (!(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) {
			event.getController()
					.setAnimation(RawAnimation.begin().then("soul_wizard_fly", PLAY_ONCE));
		} else {
			event.getController()
					.setAnimation(RawAnimation.begin().then("soul_wizard_idle", PLAY_ONCE));
		}
		return PlayState.CONTINUE;
	}

	public static AttributeSupplier.Builder setCustomAttributes() {
		return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 20.0D).add(Attributes.FLYING_SPEED, 0.15F)
				.add(Attributes.MOVEMENT_SPEED, 0.15F).add(Attributes.ATTACK_DAMAGE, 2.0D)
	...
	class ShootAttackGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && mob.distanceTo(target) <= 7.5 && mob.hasLineOfSight(target)
					&& animationsUseable();
		}
		...
		public void tick() {
			...
			if (target != null && mob.shootAnimationTick == mob.shootAnimationActionPoint) {
				...
				SoulWizardOrbEntity soulWizardOrb = new SoulWizardOrbEntity(mob.level(), mob, d1, d2,
						d3);
