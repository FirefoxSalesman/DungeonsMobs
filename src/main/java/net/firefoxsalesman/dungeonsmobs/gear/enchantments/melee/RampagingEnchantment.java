...
import static net.firefoxsalesman.dungeonsmobs.gear.config.DungeonsGearConfig.RAMPAGING_CHANCE;
import static net.firefoxsalesman.dungeonsmobs.gear.config.DungeonsGearConfig.RAMPAGING_DURATION;
...
public class RampagingEnchantment extends DungeonsEnchantment {
	...
	private static void applyEnchantment(LivingEntity attacker) {
		...
		if (ModEnchantmentHelper.hasEnchantment(mainhand, EnchantmentInit.RAMPAGING.get())) {
			int rampagingLevel = EnchantmentHelper.getItemEnchantmentLevel(EnchantmentInit.RAMPAGING.get(),
					mainhand);
	...
	private static void applyEffect(LivingEntity entity, int rampagingLevel) {
		...
		if (rampagingRand <= RAMPAGING_CHANCE.get()) {
			entity.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED,
					rampagingLevel * RAMPAGING_DURATION.get(), 4));
		}
	}
