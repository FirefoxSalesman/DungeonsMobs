	...
	public static void onRushdownKill(LivingDeathEvent event) {
		...
		if (event.getSource().getEntity() instanceof LivingEntity) {
			...
			if (ModEnchantmentHelper.hasEnchantment(mainhand, EnchantmentInit.RUSHDOWN.get())) {
				int rushdownLevel = EnchantmentHelper
						.getItemEnchantmentLevel(EnchantmentInit.RUSHDOWN.get(), mainhand);
				MobEffectInstance speed = new MobEffectInstance(MobEffects.MOVEMENT_SPEED,
						rushdownLevel * 20, 4);
