...
import net.firefoxsalesman.dungeonsmobs.gear.enchantments.types.IEmeraldsEnchantment;
...
public class DeathBarterEnchantment extends DungeonsEnchantment implements IEmeraldsEnchantment {
	...
	public static void onDeathBarter(LivingDeathEvent event) {
		...
		int deathBarterLevel = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.DEATH_BARTER.get(),
				player);
		int emeraldRequirement = 150 - Math.min(100, 50 * (deathBarterLevel - 1)); // will always need at least
												// 50 emeralds even if
												// the level exceeds 3
	...
	public boolean checkCompatibility(Enchantment enchantment) {
		return !(enchantment instanceof IEmeraldsEnchantment);
	}
