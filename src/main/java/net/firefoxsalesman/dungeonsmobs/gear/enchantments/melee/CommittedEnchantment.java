...
import static net.firefoxsalesman.dungeonsmobs.gear.config.DungeonsGearConfig.COMMITTED_BASE_MULTIPLIER;
import static net.firefoxsalesman.dungeonsmobs.gear.config.DungeonsGearConfig.COMMITTED_MULTIPLIER_PER_LEVEL;
	...
	public static void onCommittedDamage(LivingDamageEvent event) {
		...
		if (event.getSource().getEntity() instanceof LivingEntity) {
			...
			if ((ModEnchantmentHelper.hasEnchantment(mainhand, EnchantmentInit.COMMITTED.get()))) {
				int committedLevel = EnchantmentHelper
						.getItemEnchantmentLevel(EnchantmentInit.COMMITTED.get(), mainhand);
				float victimRemainingHealth = victim.getHealth() / victim.getMaxHealth();
				float originalDamage = event.getAmount();
				// If normal damage is X, the same weapon with Tier 3 Committed adds an extra (X
				// * (1 - (Mob Remaining HP/Mob Max HP))) damage.
				float extraDamageMultiplier = (float) (COMMITTED_BASE_MULTIPLIER.get()
						+ committedLevel * COMMITTED_MULTIPLIER_PER_LEVEL.get());
				event.setAmount(originalDamage
						* (1 + ((extraDamageMultiplier - 1) * (1 - victimRemainingHealth))));
			}
		}
	}
