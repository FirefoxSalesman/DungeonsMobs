...
public class EchoEnchantment extends AOEDamageEnchantment {
	...
	public static void onVanillaCriticalHit(CriticalHitEvent event) {
		if (event.getTarget() instanceof LivingEntity && !event.getEntity().level().isClientSide()
			...
			if (ModEnchantmentHelper.hasEnchantment(mainhand, EnchantmentInit.ECHO.get())) {
				int echoLevel = EnchantmentHelper.getItemEnchantmentLevel(EnchantmentInit.ECHO.get(),
						mainhand);
				...
				// AreaOfEffectHelper.causeEchoAttack(attacker, victim, attackDamage, 3.0f,
				// echoLevel);
			}
		}
	}
	...
	private static void echo(Player user, LivingEntity target, int cooldown) {
		...
		if (comboCap.getEchoCooldown() == 0 || comboCap.getEchoCooldown() == cooldown) {
			((LivingEntityAccessor) user)
					.setAttackStrengthTicker((int) (user.getAttributeValue(Attributes.ATTACK_SPEED)
							* comboCap.getCachedCooldown() * 20));
