package net.firefoxsalesman.dungeonsmobs.gear.network.entity;
...
public class PlayerBeamMessage {
	...
	public PlayerBeamMessage(int beamEntityID, double positionX, double positionY, double positionZ, float xRot,
			float yRot, float xRotO, float yRotO) {
				...
				ctx.get().enqueueWork(() -> {
					...
					if (player != null) {
						...
						if (entity instanceof ArtifactBeamEntity) {
							...
							if (artifactBeamEntity.getOwner() != player)
								return;
							artifactBeamEntity.setPos(packet.positionX, packet.positionY,
									packet.positionZ);
