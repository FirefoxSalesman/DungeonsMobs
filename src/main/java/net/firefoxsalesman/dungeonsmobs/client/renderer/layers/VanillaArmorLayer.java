...
/**
 * An armor layer for {@code ConvenientModel}. Borrowed from the
 * {@code HumanoidArmorLayer}.
 */

@OnlyIn(Dist.CLIENT)
public class VanillaArmorLayer<T extends LivingEntity, M extends ConvenientModel<T> & IArmorWearer, A extends ConvenientModel<T> & IArmorWearer>
	...
	public VanillaArmorLayer(RenderLayerParent<T, M> pRenderer, A pInnerModel, A pOuterModel,
	...
	private void renderArmorPiece(PoseStack pPoseStack, MultiBufferSource pBuffer, T pLivingEntity,
			EquipmentSlot pSlot, int pPackedLight, A pModel) {
		...
		if ($$9 instanceof ArmorItem armoritem) {
			if (armoritem.getEquipmentSlot() == pSlot) {
				...
				net.minecraft.client.model.Model model = getArmorModelHook(pLivingEntity, itemstack,
						pSlot, pModel);
				boolean flag = this.usesInnerModel(pSlot);
				if (armoritem instanceof net.minecraft.world.item.DyeableLeatherItem) {
					int i = ((net.minecraft.world.item.DyeableLeatherItem) armoritem)
							.getColor(itemstack);
					float f = (float) (i >> 16 & 255) / 255.0F;
					float f1 = (float) (i >> 8 & 255) / 255.0F;
					float f2 = (float) (i & 255) / 255.0F;
					this.renderModel(pPoseStack, pBuffer, pPackedLight, armoritem, model, flag, f,
							f1, f2,
							this.getArmorResource(pLivingEntity, itemstack, pSlot, null));
					this.renderModel(pPoseStack, pBuffer, pPackedLight, armoritem, model, flag,
							1.0F, 1.0F, 1.0F, this.getArmorResource(pLivingEntity,
									itemstack, pSlot, "overlay"));
				} else {
					this.renderModel(pPoseStack, pBuffer, pPackedLight, armoritem, model, flag,
							1.0F, 1.0F, 1.0F,
							this.getArmorResource(pLivingEntity, itemstack, pSlot, null));
				...
				ArmorTrim.getTrim(pLivingEntity.level().registryAccess(), itemstack)
						.ifPresent((p_289638_) -> {
							this.renderTrim(armoritem.getMaterial(), pPoseStack, pBuffer,
									pPackedLight, p_289638_, model, flag);
						});
	...
	protected void setPartVisibility(A pModel, EquipmentSlot pSlot) {
		...
		switch (pSlot) {
			case HEAD:
				pModel.getHead().visible = true;
				// pModel.hat.visible = true;
				break;
			case CHEST:
				pModel.getBody().visible = true;
				pModel.getRightArm().visible = true;
				pModel.getLeftArm().visible = true;
				break;
			case LEGS:
				pModel.getBody().visible = true;
				pModel.getRightLeg().visible = true;
				pModel.getLeftLeg().visible = true;
				break;
			case FEET:
				pModel.getRightLeg().visible = true;
				pModel.getLeftLeg().visible = true;
	...
	private void renderModel(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight,
			ArmorItem pArmorItem, A pModel, boolean pWithGlint, float pRed, float pGreen, float pBlue,
			@Nullable String pArmorSuffix) {
		renderModel(pPoseStack, pBuffer, pPackedLight, pArmorItem, pModel, pWithGlint, pRed, pGreen, pBlue,
				this.getArmorLocation(pArmorItem, pWithGlint, pArmorSuffix));
	}

	private void renderModel(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight,
			ArmorItem pArmorItem, net.minecraft.client.model.Model pModel, boolean pWithGlint, float pRed,
			float pGreen, float pBlue, ResourceLocation armorResource) {
		VertexConsumer vertexconsumer = pBuffer.getBuffer(RenderType.armorCutoutNoCull(armorResource));
		pModel.renderToBuffer(pPoseStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, pRed, pGreen,
				pBlue, 1.0F);
	}

	private void renderTrim(ArmorMaterial pArmorMaterial, PoseStack pPoseStack, MultiBufferSource pBuffer,
			int pPackedLight, ArmorTrim pTrim, A pModel, boolean pInnerTexture) {
		renderTrim(pArmorMaterial, pPoseStack, pBuffer, pPackedLight, pTrim, pModel, pInnerTexture);
	}

	private void renderTrim(ArmorMaterial pArmorMaterial, PoseStack pPoseStack, MultiBufferSource pBuffer,
			int pPackedLight, ArmorTrim pTrim, net.minecraft.client.model.Model pModel,
			boolean pInnerTexture) {
		TextureAtlasSprite textureatlassprite = this.armorTrimAtlas
				.getSprite(pInnerTexture ? pTrim.innerTexture(pArmorMaterial)
						: pTrim.outerTexture(pArmorMaterial));
		VertexConsumer vertexconsumer = textureatlassprite.wrap(pBuffer.getBuffer(Sheets.armorTrimsSheet()));
		pModel.renderToBuffer(pPoseStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F,
				1.0F, 1.0F);
	}
	...
	private void renderGlint(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight,
			net.minecraft.client.model.Model pModel) {
		pModel.renderToBuffer(pPoseStack, pBuffer.getBuffer(RenderType.armorEntityGlint()), pPackedLight,
	 ...
	 */
	protected Model getArmorModelHook(T entity, ItemStack itemStack, EquipmentSlot slot,
			A model) {
		A replacement = model;
		if (replacement != model) {

			model.copyPropertiesTo(replacement);
			replacement.getHead().visible = model.getHead().visible;
			replacement.getBody().visible = model.getBody().visible;
			replacement.getRightArm().visible = model.getRightArm().visible;
			replacement.getLeftArm().visible = model.getLeftArm().visible;
			replacement.getRightLeg().visible = model.getRightLeg().visible;
			replacement.getLeftLeg().visible = model.getLeftLeg().visible;
			return replacement;
		}
		return model;
	}
	 ...
	 */
	public ResourceLocation getArmorResource(net.minecraft.world.entity.Entity entity, ItemStack stack,
		...
		String s1 = String.format(java.util.Locale.ROOT, "%s:textures/models/armor/%s_layer_%d%s.png", domain,
