...
public class ArmoredPillagerModel<T extends AbstractArmouredPillagerEntity> extends ConvenientModel<T>
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
		...
		if (this.riding) {
			...
			this.right_arm.xRot = Mth.cos(limbSwing * 0.6662F + (float) Math.PI) * 2.0F * limbSwingAmount
					* 0.5F;
			this.right_arm.yRot = 0.0F;
			this.right_arm.zRot = 0.0F;
		...
		if (pose == AbstractIllager.IllagerArmPose.ATTACKING) {
			if (entity.getMainHandItem().isEmpty()) {
				AnimationUtils.animateZombieArms(this.left_arm, this.right_arm, true, this.attackTime,
						ageInTicks);
			} else {
				AnimationUtils.swingWeaponDown(this.right_arm, this.left_arm, entity, this.attackTime,
						ageInTicks);
			}
